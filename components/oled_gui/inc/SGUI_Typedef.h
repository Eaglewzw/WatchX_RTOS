...
SGUI_DEVPF_IF_DEFINE(SGUI_SIZE,         SGUI_FN_IF_GET_DATA,                (SGUI_CHAR charCode, SGUI_CBYTE* sStartAddr, SGUI_BYTE* pDataBuffer, SGUI_SIZE sReadSize));
//SGUI_DEVPF_IF_DEFINE(SGUI_BOOL,         SGUI_FN_IF_IS_FULL_WIDTH,           (SGUI_UINT32 uiCode));
SGUI_DEVPF_IF_DEFINE(SGUI_INT,          SGUI_FN_IF_GET_FONT_WIDTH,          (SGUI_CBYTE* sStartAddr, SGUI_CHAR charCode));
...
typedef struct
{
    ...
    SGUI_FN_IF_REFRESH					fnSyncBuffer; //定义函数指针变量
}SGUI_SCR_DEV;

typedef struct
{
	SGUI_CBYTE*                         fnAddress;
	SGUI_FN_IF_GET_FONT_WIDTH           fnGetWidth;
    SGUI_INT							iHeight;
